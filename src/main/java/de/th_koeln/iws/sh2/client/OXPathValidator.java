...
public class OXPathValidator implements EntryPoint {
	...
	/**
	 * Create a remote service proxy to talk to the server-side Parse service.
	 */
	private final ParseServiceAsync parseService = GWT.create(ParseService.class);
	...
	public void onModuleLoad() {
		...
		inputField.setPixelSize(300, 200);
		inputField.setText("Enter OXPath expression here");
		...
		RootPanel.get("nameFieldContainer").add(inputField);
				...
				OXPathValidator.this.parseService.parseInput(textToServer, new AsyncCallback<ParseResult>() {
					...
					public void onSuccess(ParseResult result) {
						...
						if(result.getError() != null) {
							serverResponseLabel.setText(result.getMessage() + "\n\n" + result.getError());
						}
						else serverResponseLabel.setText(result.getMessage());
						dialogBox.center();
						closeButton.setFocus(true);
					}
				});
